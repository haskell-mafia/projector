...
-- FIX should be able to do this with apE
subsToCtx :: Ground l => Subs l -> Check l (Ctx l)
subsToCtx (Subs ss) =
  fmap Ctx (traverse (typeCheck' mempty) ss) -- should this be a fold?
...
typeCheck' ctx expr =
  case expr of
    ...
    ELam ss1 n ta e -> do
      ss1' <- subsToCtx ss1
      tb <- typeCheck' (cextend n ta (ss1' <> ctx)) e
      pure (TArrow ta tb)
